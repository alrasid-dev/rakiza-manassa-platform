...
import { AlertCircle, AlertTriangle, ChevronLeft, ChevronRight, CheckCircle2, CircleDashed, Copy, Download, FilePlus2, FileText, ListChecks, Loader2, MessageCircle, Paperclip, Play, RefreshCcw, RotateCw, Search, Send, ShieldAlert, Sparkles, UserRoundCheck, ZoomIn, ZoomOut } from "lucide-react";
...
export default function TasksWorkspaceContent() {
  ...
  const addTaskCommentMutation = trpc.court.tasks.addComment.useMutation({
    onSuccess: (_result, input) => { utils.court.tasks.details.invalidate({ taskId: input.taskId }); utils.court.tasks.list.invalidate(); setCommentDialog(null); setCommentText(""); toast.success("تم حفظ التعليق في جدول التعليقات."); },
  });
