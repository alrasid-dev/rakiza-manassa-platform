...
export default function TasksWorkspaceContent() {
  ...
  const createSelf = trpc.court.tasks.createSelf.useMutation({
    onSuccess: () => { utils.court.tasks.list.invalidate(); setSelfForm({ title: "", priority: "normal", taskType: "permanent", scheduledFor: "", dueAt: "" }); toast.success("تم حفظ المهمة الذاتية وإشعار المدير المباشر للمراجعة."); },
  });
  ...
  const [form, setForm] = useState({ title: "", priority: "normal" as "normal" | "high" | "critical", taskType: "permanent" as "permanent" | "urgent", assigneeProfileId: "", watcherProfileId: "", scheduledFor: "", dueAt: "", recurrence: "none" as "none" | "daily" | "weekly" | "monthly" | "custom", recurrenceEndAt: "", isConfidential: false, confidentialityExpiresAt: "" });
  const [assigneeUnitId, setAssigneeUnitId] = useState("");
  const [watcherUnitId, setWatcherUnitId] = useState("");
  const [selfForm, setSelfForm] = useState({ title: "", priority: "normal" as "normal" | "high" | "critical", taskType: "permanent" as "permanent" | "urgent", scheduledFor: "", dueAt: "" });
  ...
  const submitSelf = (event: FormEvent) => {
    event.preventDefault();
    createSelf.mutate({ title: selfForm.title, priority: selfForm.priority, taskType: selfForm.taskType, scheduledFor: new Date(selfForm.scheduledFor), dueAt: new Date(selfForm.dueAt) });
  };
